...
工单只承接普通运维人员本人已持久化的问答，形成“创建工单 -> 管理员处理 -> 用户确认或退回”的最小人工闭环。本阶段不实现知识沉淀、Dify Dataset API、派单、SLA、审批、附件、通知、搜索、删除或归档。
...
其它跳转均由后端拒绝。管理员不能创建或代确认工单；普通用户只能读取自己的工单，管理员可读取全部工单。普通用户在 `pending` 和 `processing` 状态收到的 `processing_notes`、`solution` 固定为 `null`，管理员始终可见；`resolved` 和 `closed` 状态才向创建者展示处理内容。
...
候选知识条目、管理员审核、知识编辑和 Dify Dataset API 同步属于阶段 5，本阶段不提供占位实现。
